...
export default function NewRecords() {
  ...
  //現在のURLのクエリパラメータ(?category=22のような部分)を、読み取り専用の形で取得
  const searchParams = useSearchParams()
  //searchParamsから、category, type, periodの値をそれぞれ取り出す
  const paramCategory = searchParams.get("category") ?? "allCategories"
  const paramRecordType = searchParams.get("type") ?? "allRecordType"
  const paramPeriod = searchParams.get("period") ?? "allPeriod"

  type ParamName = "category" | "type" | "period"

  const updateParam = (name: ParamName, value: string) => {
    // 現在のsearchParamsの内容をコピーして、編集可能な新しいオブジェクトnewParamsを作成
    ...
    router.push(`/mykarte/records${newParamsString ? "?" + newParamsString : ""}`)
  }
